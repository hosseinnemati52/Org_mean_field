...
def par_obj_maker(par_dum):
    ...
    v_s_list  = np.random.normal(loc=mean_v_s, scale=err_v_s, size=n_samples)
    s_list    = np.random.normal(loc=mean_S, scale=err_S, size=n_samples)
    
    mu_w_list = (0.0*s_list).copy()
    mu_c_list = (0.0*s_list).copy()
    a_w_list  = (0.0*s_list).copy()
    a_c_list  = (0.0*s_list).copy()
    r_d_list  = (0.0*s_list).copy()
    
    for i in range(n_samples):
        #par_dum.V_S   = 10.8
        par_dum.V_S   = v_s_list[i]
        
        # keep one of these
        # par_dum.S_W  = 0.7123346753965393 * 0.012 #nondim
        par_dum.S_W  = s_list[i]
        # par_dum.S_W  = 18.6
        ...
        par_dum = par_calculator(par_dum)
        
        mu_w_list[i] = par_dum.MU_W
        mu_c_list[i] = par_dum.MU_C
        a_w_list[i]  = par_dum.alpha_w
        a_c_list[i]  = par_dum.alpha_c
        r_d_list[i]  = par_dum.R_0_d
    ...
    plt.figure()
    plt.hist(v_s_list, bins=n_bins)
    
    plt.figure()
    plt.hist(s_list, bins=n_bins)
    
    plt.figure()
    plt.hist(mu_w_list, bins=n_bins)
    
    plt.figure()
    plt.hist(mu_c_list, bins=n_bins)
    
    plt.figure()
    plt.hist(a_w_list, bins=n_bins)
    
    plt.figure()
    plt.hist(a_c_list, bins=n_bins)
    
    plt.figure()
    plt.hist(r_d_list, bins=n_bins)
    
    data_to_save = np.zeros((7,2))
    data_to_save[0,:] = [mean_S, err_S]
    data_to_save[1,:] = [mean_v_s, err_v_s]
    data_to_save[2,:] = [np.mean(mu_w_list), np.std(mu_w_list)]
    data_to_save[3,:] = [np.mean(mu_c_list), np.std(mu_c_list)]
    data_to_save[4,:] = [np.mean(a_w_list), np.std(a_w_list)]
    data_to_save[5,:] = [np.mean(a_c_list), np.std(a_c_list)]
    data_to_save[6,:] = [np.mean(r_d_list), np.std(r_d_list)]
    np.savetxt("parameters_errors.csv", X= data_to_save, delimiter=',', fmt='%.5f')
