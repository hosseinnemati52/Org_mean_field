...
def par_obj_maker_mu_variations(par_dum, mu_w, mu_c):
    ...
    A = 1
    B = par_dum.MU_W*( 1 + par_dum.S_W / par_dum.V_S)
    C = - par_dum.MU_W * par_dum.S_W * np.log(2) / np.pi
    delta = B**2 - 4*A*C
    par_dum.gamma_w = (-B + delta**0.5) / (2*A)
    
    
    par_dum.alpha_w = par_dum.S_W / (par_dum.gamma_w+par_dum.MU_W)
    # from pure WT
