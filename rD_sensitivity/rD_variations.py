...
def pop_sol_creator(par_dum, sol_dum):
    
    pop_sol_dum = pop_sol_class()
    pop_sol_dum.time = sol_dum.time_tilde/par_dum.beta_wt
    
    pop_sol_dum.eval_time = 60
    pop_sol_dum.eval_ind = np.argmin(np.abs(pop_sol_dum.time-pop_sol_dum.eval_time))
    pop_sol_dum.g0_frac_list = []
    
    pop_sol_dum.pure_w = sol_dum.pure_w_sol.n_w_tot
    pop_sol_dum.pure_c = sol_dum.pure_c_sol.n_c_tot

    n_org = par_dum.n_org
    
    n_w_mix_mat = np.zeros((n_org, sol_dum.L))
    n_c_mix_mat = np.zeros((n_org, sol_dum.L))
    
    g0_frac_temporal_mix_mat = np.zeros((n_org, sol_dum.L))
    for org_c in range(n_org):
        n_w_mix_mat[org_c,:] = sol_dum.mixed_sol[org_c].n_w_tot / sol_dum.mixed_sol[org_c].n_w_tot[0]
        n_c_mix_mat[org_c,:] = sol_dum.mixed_sol[org_c].n_c_tot / sol_dum.mixed_sol[org_c].n_c_tot[0]
        
        g0_frac_temporal_mix_mat[org_c,:] = sol_dum.mixed_sol[org_c].n_w_d / sol_dum.mixed_sol[org_c].n_w_tot
        
        pop_sol_dum.g0_frac_list.append(sol_dum.mixed_sol[org_c].n_w_d[pop_sol_dum.eval_ind] / sol_dum.mixed_sol[org_c].n_w_tot[pop_sol_dum.eval_ind])
    
    pop_sol_dum.mix_w_avg = np.mean(n_w_mix_mat, axis=0)
    pop_sol_dum.mix_w_sem = np.std(n_w_mix_mat, axis=0)/np.sqrt(n_org)
    
    pop_sol_dum.mix_c_avg = np.mean(n_c_mix_mat, axis=0)
    pop_sol_dum.mix_c_sem = np.std(n_c_mix_mat, axis=0)/np.sqrt(n_org)
    
    pop_sol_dum.g0_frac_avg = np.mean(pop_sol_dum.g0_frac_list) # only at t = 60 h
    pop_sol_dum.g0_frac_sem = np.std(pop_sol_dum.g0_frac_list)/np.sqrt(n_org) # only at t = 60 h
    
    pop_sol_dum.g0_frac_temporal_avg = np.mean(g0_frac_temporal_mix_mat, axis=0) # temporal 
    pop_sol_dum.g0_frac_temporal_sem = np.std(g0_frac_temporal_mix_mat, axis=0)/np.sqrt(n_org) # temporal
    
    return pop_sol_dum

def sol_population_plotter_mu_variation(par_list, sol_list):
...
def sol_obj_maker(sol_dum, par_dum):
    ...
    ## mesh
    sol_dum.n_interval_G1 = 100
    sol_dum.n_interval_S  = 100
...
def par_obj_maker(par_dum):
    ...
    # keep one of these
    # par_dum.S_W  = 0.7123346753965393 * 0.012 #nondim
    par_dum.S_W  = 19.57
    # par_dum.S_W  = 18.6
    # par_dum.MU_W = 1970 #nondim
    # keep one of these
    
    # keep two of these
    # par_dum.S_C  = 0.7123346753965393 * 0.012 #nondim
    par_dum.S_C  = 19.57
    # par_dum.S_C  = 18.6
    # par_dum.MU_C = 7665 #nondim
    par_dum.V_0   = 0
    # keep two of these
...
def par_obj_maker_mu_variations(par_dum, mu_w, mu_c):
    ...
    par_dum.S_W = (1+par_dum.MU_W)*(1+par_dum.R_0_d)/(m*par_dum.MU_W)
    ...
    # from pure C
    par_dum.S_C = par_dum.S_W
    par_dum.MU_C = mu_c
    
    A = 1
    B = par_dum.MU_C*( 1 + par_dum.S_C / par_dum.V_S)
    C = - par_dum.MU_C * par_dum.S_C * np.log(2) / np.pi
    delta = B**2 - 4*A*C
    par_dum.gamma = (-B + delta**0.5) / (2*A)
    ...
    print("resid for C:")
    print(par_dum.gamma *(par_dum.gamma + par_dum.MU_C) / (par_dum.MU_C*par_dum.S_C) + par_dum.gamma / par_dum.V_S - np.log(2) / np.pi)
    print("resid for W:")
    print((1+par_dum.R_0_d) *(1 + par_dum.MU_W) / (par_dum.MU_W*par_dum.S_W) + 1 / par_dum.V_S - np.log(2) / np.pi)
...
def par_obj_maker_rD_variations(par_dum, r_D_dum, r_mu_dum):
    
    
    par_dum.n_org = 100
    # par_dum.beta_wt = 0.0284
    par_dum.tau = (1.0/0.0284)
    par_dum.beta_wt = 1 / par_dum.tau
    par_dum.t_eval = 60
    
    # from pure WT
    par_dum.S_W = par_vals[0]
    par_dum.MU_W = par_vals[3]
    par_dum.V_S   = par_vals[2]
    par_dum.V_0   = 0
    ...
    # par_dum.R_0_d = par_vals[7]
    par_dum.R_0_d = r_D_dum
    
    A = 1
    B = par_dum.R_0_d+par_dum.MU_W+par_dum.MU_W*par_dum.S_W/par_dum.V_S
    C = par_dum.MU_W*par_dum.R_0_d-par_dum.MU_W*par_dum.S_W*np.log(2)/np.pi
    delta = B**2 - 4*A*C
    par_dum.eta = (-B + delta**0.5) / (2*A)
    
    
    m = np.log(2)/np.pi - par_dum.eta / par_dum.V_S # doubling BC of WT
    m_prime = par_dum.eta / par_dum.V_S
    ...
    par_dum.f_D_W =  1 /  ( ((par_dum.eta+par_dum.R_0_d)*(1-np.exp(-m_prime*np.pi))/(np.exp(m*np.pi)-1)+par_dum.eta)/par_dum.R_0_d + 1 )
    # par_dum.f_D_W =  0.99
    par_dum.f_P_W =  1- par_dum.f_D_W
    ...
    # par_dum.alpha_w = par_dum.S_W / (1+par_dum.MU_W)
    par_dum.alpha_w = par_dum.S_W / (par_dum.eta+par_dum.MU_W)
    # par_dum.alpha_w = 14.57
    # from pure WT
    ...
    par_dum.S_C = par_dum.S_W
    # par_dum.MU_C = mu_c
    # par_dum.MU_C = par_vals[4]
    
    par_dum.MU_C = r_mu_dum * par_dum.MU_W
...
def total_ploter_pureC(par_list, pop_sol_list):
    ...
    # # plt.scatter(WT_mix_exp[0,:], WT_mix_exp[1,:], label='exp_WT_mix')
    # plt.errorbar(WT_mix_exp[0,:], WT_mix_exp[1,:], yerr=WT_mix_exp[2,:], fmt='s', color='m', ecolor='m', capsize=2, label='mixed WT (exp)', markerfacecolor='none', zorder=10)
    plt.errorbar([-1], [-1], yerr=[1], fmt='s', color='m', ecolor='m', capsize=2, label='mixed WT (exp)', markerfacecolor='none', zorder=10)
    
    # #C_mix
    # # plt.scatter(C_mix_exp[0,:], C_mix_exp[1,:], label='exp_C_mix')
    # plt.errorbar(C_mix_exp[0,:], C_mix_exp[1,:], yerr=C_mix_exp[2,:], fmt='s', color='g', ecolor='g', capsize=2, label='mixed C (exp)', markerfacecolor='none', zorder=10)
    plt.errorbar([-1], [-1], yerr=[1], fmt='s', color='g', ecolor='g', capsize=2, label='mixed C (exp)', markerfacecolor='none', zorder=10)
    ...
    # color_start = "#1f77b4"  # blue
    # color_end   = "#d62728"  # red
    
    # Create the colormap
    # cmap = LinearSegmentedColormap.from_list("BlueRed", [color_start, color_end])
    ...
    min_val = np.log(par_list[0].MU_C / par_list[0].MU_W)
    max_val = np.log(par_list[-1].MU_C / par_list[-1].MU_W)
    norm = mcolors.Normalize(vmin=min_val, vmax=max_val)

    # plt.plot(pop_sol_list[0].time, pop_sol_list[0].pure_w, label='pure WT (model)', color = 'k', linestyle='dotted')
    plt.plot(pop_sol_list[0].time, pop_sol_list[0].pure_w, label='pure WT', color = 'k', linestyle='dashed')
    
    for i in range(len(par_list)):
        
        value = np.log(par_list[i].MU_C / par_list[i].MU_W)
        color = cmap(norm(value))
        
        # plt.plot(pop_sol_list[i].time, pop_sol_list[i].pure_w, label='pure WT (model)', color = color, linestyle='dotted')
        # plt.plot(pop_sol_list[i].time, pop_sol_list[i].pure_c, label='pure C (model)', color = color, linestyle='dotted')
        
        # label = 'pure C ('+r'$\tilde{\mu}_\mathrm{C}/\tilde{\mu}_\mathrm{W} = $'+str(round(value,2)) + ')'
        # label = 'pure C ('+r'$r_{\mu} = $'+str(round(np.exp(value),2)) + ')'
        label = r'$r_{\mu} = $'+str(round(np.exp(value),2))
        
        if np.abs(np.exp(value) - 3.9188) <0.1:
            plt.plot(pop_sol_list[i].time, pop_sol_list[i].pure_c, label=label, color = color, linestyle='solid')
        else:
            plt.plot(pop_sol_list[i].time, pop_sol_list[i].pure_c, label=label, color = color, linestyle='solid')
            
        
        # plt.errorbar(pop_sol_list[i].time, pop_sol_list[i].mix_w_avg, yerr=pop_sol_list[i].mix_w_sem, capsize=0, label='mixed W (model)', color = color, alpha=0.6)
        # plt.errorbar(pop_sol_list[i].time, pop_sol_list[i].mix_c_avg, yerr=pop_sol_list[i].mix_c_sem, capsize=0, label='mixed C (model)', color = color, alpha=0.6)
    
    
    time = pop_sol_list[i].time
    plt.xlabel('time(h)', fontsize=15)
    # plt.ylabel(r'$\langle N(t)/N(0) \rangle$', fontsize=15)
    plt.ylabel(r'$\overline{W}_p(t)$'+', '+r'$\overline{C}_p(t)$', fontsize=15)
    # plt.grid()
    
    sm = cm.ScalarMappable(norm=norm, cmap=cmap)
    sm.set_array([])  # required before Matplotlib 3.7
    # cbar = plt.colorbar(sm, label=r'$\log(r_{\mu})$')  # or label='log(MU_C/MU_W)'
    cbar = plt.colorbar(sm, ax=plt.gca(), fraction=0.046, pad=0.04)
    cbar.set_label(r'$\ln(r_{\mu})$', fontsize=15)  # make label text bigger
    ...
    # plt.legend(loc="upper left", bbox_to_anchor=(1, 1))
    plt.legend(
    loc='lower center',          # bottom of legend box is anchored
    bbox_to_anchor=(0.5, 1.02),  # center it horizontally, just above the axes
    borderaxespad=0,             # no extra vertical padding
    ncol=3                       # arrange entries in multiple columns
    )
    # plt.legend()
    plt.tight_layout()
    plt.savefig('pops_trend_pureC.PNG', dpi=300)
    
    return

def total_ploter_mixed_C(par_list, pop_sol_list):
    ...
    pure_C_index = 0
    for i in range(len(par_list)):
        if np.abs( par_list[i].MU_C / par_list[i].MU_W - 3.9188) < 0.1:
            pure_C_index = i
    ...
    #pure WT baseline
    plt.plot(pop_sol_list[0].time, 1.0*np.ones(len(pop_sol_list[0].time)), label='pure WT', color = 'k')
    
    
    
    # capturing indices (pure WT case)
    indices = []
    for exp_time_c in range(len(WT_pure_exp[0,:])):    
        index = np.argmin(np.abs(pop_sol_list[0].time - WT_pure_exp[0,exp_time_c]))
        indices.append(index)
    indices = np.array(indices)
    
    
    
    model_at_exp_times_pure_W_avg = []
    # model_at_exp_times_pure_W_err = []
    # model_at_exp_times_pure_C_avg = []
    # model_at_exp_times_pure_C_err = []
    # model_at_exp_times_mix_W_avg = []
    # model_at_exp_times_mix_W_err = []
    # model_at_exp_times_mix_C_avg = []
    # model_at_exp_times_mix_C_err = []
    for index_c in range(len(indices)):
        index = indices[index_c]
        model_at_exp_times_pure_W_avg.append(pop_sol_list[0].pure_w[index])
        # model_at_exp_times_pure_W_err.append([index])
        # model_at_exp_times_pure_C_avg.append(pop_sol_list[pure_C_index].pure_c[index])
        # model_at_exp_times_pure_C_err.append([index])
        # model_at_exp_times_mix_W_avg.append([index])
        # model_at_exp_times_mix_W_err.append([index])
        # model_at_exp_times_mix_C_avg.append([index])
        # model_at_exp_times_mix_C_err.append([index])
        
        
    # capturing indices (pure C case)
    indices = []
    for exp_time_c in range(len(WT_pure_exp[0,:])):    
        index = np.argmin(np.abs(pop_sol_list[pure_C_index].time - WT_pure_exp[0,exp_time_c]))
        indices.append(index)
    indices = np.array(indices)
    ...
    for index_c in range(len(indices)):
        index = indices[index_c]
        # model_at_exp_times_pure_W_avg.append(pop_sol_list[pure_C_index].pure_w[index])
        # model_at_exp_times_pure_W_err.append([index])
        model_at_exp_times_pure_C_avg.append(pop_sol_list[pure_C_index].pure_c[index])
        # model_at_exp_times_pure_C_err.append([index])
        # model_at_exp_times_mix_W_avg.append([index])
    ...
    model_at_exp_times_pure_W_avg = np.array(model_at_exp_times_pure_W_avg)
    model_at_exp_times_pure_C_avg = np.array(model_at_exp_times_pure_C_avg)
    
    
    # #WT_pure
    # # plt.scatter(WT_pure_exp[0,:], WT_pure_exp[1,:], label='exp_WT_pure')
    # plt.errorbar(WT_pure_exp[0,:], WT_pure_exp[1,:]/ model_at_exp_times_pure_W_avg , yerr=WT_pure_exp[2,:]/ model_at_exp_times_pure_W_avg, fmt='o', color='m', ecolor='m', capsize=2, label='pure WT (exp)', zorder=10)
    
    
    #C_mix
    # plt.scatter(C_mix_exp[0,:], C_mix_exp[1,:], label='exp_C_mix')
    plt.errorbar(C_mix_exp[0,:], C_mix_exp[1,:]/ model_at_exp_times_pure_C_avg, yerr=C_mix_exp[2,:]/ model_at_exp_times_pure_C_avg, fmt='s', color='g', ecolor='g', capsize=2, label='mixed C (exp)', markerfacecolor='none', zorder=10)
    
    # #WT_mix
    # # plt.scatter(WT_mix_exp[0,:], WT_mix_exp[1,:], label='exp_WT_mix')
    # plt.errorbar(WT_mix_exp[0,:], WT_mix_exp[1,:]/ model_at_exp_times_pure_W_avg, yerr=WT_mix_exp[2,:]/ model_at_exp_times_pure_W_avg, fmt='s', color='m', ecolor='m', capsize=2, label='mixed WT (exp)', markerfacecolor='none', zorder=10)
    
    #C_pure
    # plt.scatter(C_pure_exp[0,:], C_pure_exp[1,:], label='exp_C_pure')
    plt.errorbar(C_pure_exp[0,:], C_pure_exp[1,:]/ model_at_exp_times_pure_C_avg, yerr=C_pure_exp[2,:]/ model_at_exp_times_pure_C_avg, fmt='o', color='g', ecolor='g', capsize=2, label='pure C (exp)', zorder=10)
    ...
    # plt.plot(pop_sol_list[0].time, pop_sol_list[0].pure_w, label='pure WT (model)', color = 'k', linestyle='dotted')
    # plt.plot(pop_sol_list[0].time, pop_sol_list[0].pure_w, label='pure WT', color = 'k')
    ...
    for i in range(len(par_list)):
        ...
        # label = 'mixed WT ('+r'$r_{\mu} = $'+str(round(np.exp(value),2)) + ')'
        # y    = pop_sol_list[i].mix_w_avg / pop_sol_list[i].pure_w
        # yerr = pop_sol_list[i].mix_w_sem / pop_sol_list[i].pure_w
        # plt.errorbar(pop_sol_list[i].time, y , yerr=yerr,  label=label, color = color, linestyle=linestyle)
            
            
            
        label = 'mixed C ('+r'$r_{\mu} = $'+str(round(np.exp(value),2)) + ')'
        y    = pop_sol_list[i].mix_c_avg / pop_sol_list[i].pure_c
        yerr = pop_sol_list[i].mix_c_sem / pop_sol_list[i].pure_c
        plt.errorbar(pop_sol_list[i].time, y , yerr=yerr,  label=label, color = color, linestyle=linestyle)
    ...
    plt.ylabel(r'$\overline{C}_m(t)/\overline{C}_p(t)$', fontsize=30)
    # plt.grid()
    ...
    # # cbar = plt.colorbar(sm, label=r'$\log(r_{\mu})$')  # or label='log(MU_C/MU_W)'
    # cbar = plt.colorbar(sm, ax=plt.gca(), fraction=0.046, pad=0.04)
    # cbar.set_label(r'$\ln(r_{\mu})$', fontsize=20)  # make label text bigger

    
    

    plt.xlim((np.min(time), np.max(time)))
    plt.xticks([0, 20, 40, 60], fontsize=30)
    plt.yticks([1.0, 1.4, 1.8], fontsize=30)
    # plt.yscale("log")
    ...
    # plt.legend()
    plt.tight_layout()
    plt.savefig('pops_trend_mixed_C.PNG', dpi=300)
    
    return

def total_ploter_mixed_W(par_list, pop_sol_list):
    ...
    # plt.scatter(C_mix_exp[0,:], C_mix_exp[1,:], label='exp_C_mix')
    # plt.errorbar(C_mix_exp[0,:], C_mix_exp[1,:]/ model_at_exp_times_pure_C_avg, yerr=C_mix_exp[2,:]/ model_at_exp_times_pure_C_avg, fmt='s', color='g', ecolor='g', capsize=2, label='mixed C (exp)', markerfacecolor='none', zorder=10)
    # plt.errorbar(None, None, yerr=None, fmt='s', color='g', ecolor='g', capsize=2, label='mixed C (exp)', markerfacecolor='none', zorder=10)
    ...
    # plt.scatter(C_pure_exp[0,:], C_pure_exp[1,:], label='exp_C_pure')
    # plt.errorbar(C_pure_exp[0,:], C_pure_exp[1,:]/ model_at_exp_times_pure_C_avg, yerr=C_pure_exp[2,:]/ model_at_exp_times_pure_C_avg, fmt='o', color='g', ecolor='g', capsize=2, label='pure C (exp)', zorder=10)
    # plt.errorbar(None, None, yerr=None, fmt='o', color='g', ecolor='g', capsize=2, label='pure C (exp)', zorder=10)
    ...
    time = pop_sol_list[i].time
    plt.xlabel('time(h)', fontsize=30)
    plt.ylabel(r'$\overline{W}_m(t)/\overline{W}_p(t)$', fontsize=30)
    # plt.grid()
    ...
    # plt.legend()
    plt.tight_layout()
    plt.savefig('pops_trend_mixed_W.PNG', dpi=300)
    ...
    yerr = []
    for i in range(len(par_list)):
        value = par_list[i].MU_C / par_list[i].MU_W
        x.append(value)
        y.append(pop_sol_list[i].g0_frac_avg)
        yerr.append(pop_sol_list[i].g0_frac_sem)
        
        if i == pure_C_index:
            plt.errorbar(x[pure_C_index],y[pure_C_index] , yerr=yerr[pure_C_index], fmt='o', markersize=10, color='m', ecolor='m', capsize=5, label='fitted', markerfacecolor='m', zorder=10)
        else:
            plt.errorbar(x[i],y[i] , yerr=yerr[i], fmt='s', color='k', ecolor='k', markersize = 10, capsize=5, label=None, markerfacecolor='none', zorder=1)
        # label = 'mixed WT ('+r'$r_{\mu} = $'+str(round(np.exp(value),2)) + ')'
    
    plt.xlabel(r'$r_{\mu}$', fontsize=30)
    plt.ylabel('G0 fraction', fontsize=30)
    ax = plt.gca()
    ax.set_xscale('log')
    ax.set_xticks([1, 2, 4, 8, 16])
    ax.set_xticklabels(['1', '2', '4', '8', '16'], fontsize=30)
    ...
    plt.savefig('G0_frac_mu_c.PNG', dpi=300)
    
    
    
    
    # temporal G0 fraction
    plt.figure()
    plt.plot(pop_sol_list[0].time, par_list[0].f_D_W*np.ones(len(pop_sol_list[0].time)), label='pure WT', color = 'k')
    for i in range(len(par_list)):
        ...
        label = 'mixed WT ('+r'$r_{\mu} = $'+str(round(np.exp(value),2)) + ')'
        y    = pop_sol_list[i].g0_frac_temporal_avg
        yerr = pop_sol_list[i].g0_frac_temporal_sem
        plt.errorbar(pop_sol_list[i].time, y , yerr=yerr,  label=label, color = color, linestyle=linestyle)
    
    # plt.errorbar([60], [0.282] , yerr=[0.021], fmt='s', color='m', ecolor='m', capsize=2, label='mixed WT (exp)', markerfacecolor='none', zorder=10)
    ...
    # plt.legend()
    plt.tight_layout()
    plt.savefig('g0_frac_mixed_wt.PNG', dpi=300)
    
    return

def total_ploter_mixed_both(par_list, pop_sol_list):
    ...
    # plt.legend()
    plt.tight_layout()
    plt.savefig('pops_trend_mixed_both.PNG', dpi=300)
...
# mu_w_orig = 1.3375821235907959
# mu_c_orig = 5.2417627221286506

par_vals = np.loadtxt("par_vals.csv", delimiter=',')
r_D_orig = par_vals[7]
MU_W_orig = par_vals[3]

# MU_C_list = [1.3375821235907959, 2.0, 3.0, 4.0, 5.2417]
# MU_C_list = [0.5, 0.8, 1.0, 1.3375821235907959, 2.0, 3.0, 4.0, 5.2417]

# MU_C_list = mu_w_orig * np.array([1.0, 2.0, 3.0, 3.9188])
# MU_C_list = mu_w_orig * np.array([1.0, 2.0, 3.9188, 8.0, 16.0])
# r_D_list = [ 0, 0.2,  r_D_orig, 1.0, 4.0, 10.0]

r_D_list = np.array(list(np.array(list(2**np.linspace(-3,0,10)[:-1])+
                            list(2**np.linspace(0,3,10))))) * r_D_orig

r_mu_list = (2**np.linspace(0,4,22))

# r_D_list = np.array(list(np.array(list(2**np.linspace(-3,0,3)[:-1])+
#                             list(2**np.linspace(0,3,3))))) * r_D_orig



# r_mu_list = (2**np.linspace(0,4,3))
...
np.savetxt('r_D_list.csv', X=r_D_list, delimiter=',')
np.savetxt('r_mu_list.csv', X=r_mu_list, delimiter=',')
# r_D_list = [ 0, 0.05, r_D_orig, 3.46]
# MU_C_list = mu_w_orig * np.array([1.0, 2.0, 3.9188, 8.0])

# MU_C_list = mu_w_orig * np.array([1.0, 3.9188])
# MU_C_list = mu_w_orig * np.array([1.0])
# MU_C_list = [1.3375821235907959]

par_list = []
pop_sol_list = []

w_p_norm_final = np.zeros((len(r_D_list), len(r_mu_list)))
c_p_norm_final = np.zeros((len(r_D_list), len(r_mu_list)))
w_m_norm_final = np.zeros((len(r_D_list), len(r_mu_list)))
c_m_norm_final = np.zeros((len(r_D_list), len(r_mu_list)))

wd_p_perc_final = np.zeros((len(r_D_list), len(r_mu_list)))
wd_m_perc_final = np.zeros((len(r_D_list), len(r_mu_list)))

par = params_class()
sol = solution_class()

for i in range(len(r_D_list)):
# for i in range(1):
    print('***')
    print(i)
    print('***')
    for j in range(len(r_mu_list)):
        ...
        # par_obj_maker_mu_variations(par, mu_w_orig, mu_c)
        par_obj_maker_rD_variations(par, r_D_list[i], r_mu_list[j])
        # par_obj_maker_rD_variations(par, None)
        
        save_object(par, 'par_'+str(i)+'_'+str(j)+'.txt')
        ...
        try:
            sample_indices_mix = np.loadtxt('sample_indices_mix.csv', delimiter=',' , dtype=int)
        except:
            mixed_sample_bank = np.loadtxt("mixed_sample_bank.csv", delimiter=',', dtype=int)
            size = np.shape(mixed_sample_bank)[0]
        ...
        par_list.append(copy.deepcopy(par))
        pop_sol = pop_sol_creator(par, sol)
        pop_sol_list.append(copy.deepcopy(pop_sol))
        
        
        w_p_norm_final[i,j] = WT_pure_model[1,-1]
        c_p_norm_final[i,j] = C_pure_model[1,-1]
        w_m_norm_final[i,j] = WT_mix_model[1,-1]
        c_m_norm_final[i,j] = C_mix_model[1,-1]
        
        wd_p_perc_final[i,j] = (sol.pure_w_sol.n_w_d/sol.pure_w_sol.n_w_tot)[-1]
        
        wd_m_perc_final[i,j] = 0.0
        for org_c in range(par.n_org):
            wd_m_perc_final[i,j] += ( (sol.mixed_sol[org_c].n_w_d/sol.mixed_sol[org_c].n_w_tot)[-1] / par.n_org )
        ...
        np.savetxt('w_p_norm_final.csv', X=w_p_norm_final, delimiter=',')
        np.savetxt('c_p_norm_final.csv', X=c_p_norm_final, delimiter=',')
        np.savetxt('w_m_norm_final.csv', X=w_m_norm_final, delimiter=',')
        np.savetxt('c_m_norm_final.csv', X=c_m_norm_final, delimiter=',')
        np.savetxt('wd_p_perc_final.csv', X=wd_p_perc_final, delimiter=',')
        np.savetxt('wd_m_perc_final.csv', X=wd_m_perc_final, delimiter=',')

# total_ploter_pureC(par_list, pop_sol_list)

# total_ploter_mixed_C(par_list, pop_sol_list)

# total_ploter_mixed_W(par_list, pop_sol_list)

# total_ploter_mixed_both(par_list, pop_sol_list)
