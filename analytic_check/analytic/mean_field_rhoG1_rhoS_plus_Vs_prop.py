...
def rho_solver(par, sol, rho_G1_0_0_w, rho_G1_0_0_c, n_0_d, M_0):
    ...
    AA = (1-np.exp(-par.m*np.pi))/par.m + par.MU_W*par.alpha_w*np.exp(-par.m*np.pi)*(1-np.exp(-np.pi/par.V_S))
    rho_0_W = (1-par.f_D_W)*n_w_tot[0] * (AA)**(-1)
    rho_pi_W = rho_0_W * np.exp(-par.m*np.pi) * par.MU_W * par.alpha_w / par.V_S
    rho_0_C = 2 * n_c_tot[0] * ( np.log(2)/np.pi - par.gamma / par.V_S )
    rho_pi_C = rho_0_C * np.exp(par.gamma * np.pi / par.V_S) * par.MU_C * par.alpha_c / (2*par.V_S)
    
    for dt_c in range(1,L):
        
        t_tilde = sol.time_tilde[dt_c]
        # if dt_c > n_ind_delay:
        ...
        #updating WT in G1
        rho_G1_W[dt_c, :] = rho_0_W * np.exp(-par.m * sol.x_G1) * np.exp(t_tilde)
        #updating C in G1
        rho_G1_C[dt_c, :] = rho_0_C * np.exp(-par.q * sol.x_G1) * np.exp(par.gamma * t_tilde)
        #updating WT in S
        rho_S_W[dt_c, :]  = rho_pi_W * np.exp(-(sol.x_S-np.pi)/par.V_S) * np.exp(t_tilde)
        #updating C in S
        rho_S_C[dt_c, :]  = rho_pi_C * np.exp(-par.gamma*(sol.x_S-np.pi)/par.V_S) * np.exp(par.gamma * t_tilde)
        
        
        #updating diff WT
        # n_w_d[dt_c]  =  n_w_d[dt_c-1] + dt * ( (par.R_0_d + par.R_cd * n_c_tot[dt_c-1]) * n_w_g1[dt_c-1] - par.R_a * n_w_d[dt_c-1] * n_c_tot[dt_c-1] )
        n_w_d[dt_c]  =  par.f_D_W * n_w_tot[0] * np.exp(t_tilde)
        ...
        if n_c_tot[0]<0.00001:
            M[dt_c] = n_w_tot[0] * par.alpha_w * np.exp(t_tilde)
        elif n_w_tot[0]<0.00001:
            M[dt_c] = n_c_tot[0] * par.alpha_c * np.exp(par.gamma * t_tilde)
        else:
            M[dt_c] = M[dt_c-1] + dt * ( par.S_W * n_w_tot[dt_c-1] + par.S_C * n_c_tot[dt_c-1] - n_w_tot[dt_c-1] * par.MU_W * concen[dt_c-1] - n_c_tot[dt_c-1] *  par.MU_C * concen[dt_c-1] )
